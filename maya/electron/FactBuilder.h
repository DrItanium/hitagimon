...
#ifndef __LibElectron_FactBuilder_h__
#define __LibElectron_FactBuilder_h__
#include "electron/Environment.h"

namespace Electron 
{
/**
 * Programatic equivalent to assert for a deftemplate
 */
class FactBuilder
{
public:
    using RawType = ::FactBuilder;
    using ErrorCode = ::FactBuilderError;
public:
    FactBuilder(Environment& env, const std::string& templateName);
    ~FactBuilder();
    Fact* make();
    template<typename T>
    ErrorCode make(T* out)
    {
        out->factValue = make();
        return error();
    }
...
#endif // end __LibElectron_FactBuilder_h__
