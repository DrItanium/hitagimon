/**
 * @file
 * Modify clips instances programatically
 * @copyright
...
#ifndef __LibElectron_InstanceModifier_h__
#define __LibElectron_InstanceModifier_h__
#include "electron/Environment.h"

namespace Electron 
{
/**
 * Programatic equivalent to modify-instance
 */
class InstanceModifier
{
public:
    using RawType = ::InstanceModifier;
    using ErrorCode = ::InstanceModifierError;
public:
    InstanceModifier(Environment& env, Instance* target);
    ~InstanceModifier();
    Instance* modify();
    template<typename T>
    ErrorCode modify(T* out)
    {
        out->instanceValue = modify();
        return error();
    }
    void abort();
    ErrorCode error();
...
#endif // end __LibElectron_InstanceModifier_h__
